  ...
  '''
    ...
    for batch in batches:
        ...
        for name in batch[0]:
            ...
            details = [0.] * 13  # Create a one-hot that contains zero (only for age).
            index = mapAgeIntoClass(int(age_gender_color_by_regex.group(1).replace("_", "")))
            details[index] = 1.  # We want only the age_class.
...
if __name__ == '__main__':
    ...
    x_train, _, x_test = split_train_test(batches, 0.85, 0.0, 0.15)
    ...
    age_loss = tf.reduce_mean(tf.pow(age - X_age, 2), name = "age_loss")
 
    optimizer = tf.train.AdamOptimizer(0.0005).minimize(age_loss)
    ...
    with tf.Session() as sess:
        ...
        for i in range(80001):
            ...
            details_train = details_train.reshape([-1, 13])
 
            sess.run(optimizer, feed_dict={X_in: batch_train, X_age: details_train, keep_prob: 0.8})
            ...
            if i % 200 == 0:
                ...
                details_test = details_test.reshape([-1, 13])
                ...
                print('Iteration:', i, ' loss:',
                      age_loss.eval(feed_dict={X_in: batch_test, X_age: details_test, keep_prob: 1.0}))
                print("\n\n\n")
                k_test += 1
 
                if local_loss > loss * 1.4 and loss < 200:
                    os.mkdir(str(loss) + "_Age_prediction")
                    # Save the variable in the file
                    saved_path = saver.save(sess, str(loss) + "_Age_prediction" + '/saved_variable')
