  ...
  '''
    ...
    for batch in batches:
        ...
        for name in batch[0]:
            ...
            index = (int(age_gender_color_by_regex.group(2).replace("_", "")))
           # details = [0.] * 2  # Create a one-hot that contains zero (only for age).
            details=index  #
...
def encoder(X_in, keep_prob):
  ...
  '''
    ...
    with tf.variable_scope("encoder", reuse=None):
        ...
        logits = tf.layers.dense(x, units=1, name="encoder_dense_1")
...
if __name__ == '__main__':
    ...
    categories = 1 #for logistic

    X_in = tf.placeholder(dtype=tf.float32, shape=[None, heigh_global, width_global, 1], name='X')
    X_gender = tf.placeholder(dtype=tf.float32, shape=[None, categories], name='true_gender')  # 0 - men , 1-wonem
    keep_prob = tf.placeholder(dtype=tf.float32, shape=(), name='keep_prob')

    dec_in_channels = 1  # RGB - 3 Grayscale - 1

    gender = encoder(X_in, keep_prob)

    gender=tf.nn.sigmoid(gender)
    loss1 = tf.losses.log_loss(labels=X_gender, predictions=gender, epsilon=1e-6)
    gender_loss = tf.reduce_mean(loss1)

    optimizer = tf.train.AdamOptimizer(0.0005).minimize(gender_loss)

    correct_prediction = tf.equal(X_gender,tf.round(gender))
...
0.8864516115957691_Genders_prediction_ephoch_5
        ...
        for i in range(len(batches)):
            ...
            loss, genders, local_acc = sess.run([gender_loss, gender, accuracy],
                                                    feed_dict={X_in: batch_test, X_gender: details_test,
                                                               keep_prob: 1.0})

            total_acc_per_model += local_acc
            print("The gender:")
            print(sess.run(tf.round(genders)))
            print("\n")
            print("The original:")
            print(details_test)
            print("\n")
    ...
    print("The acc:")
    print((total_acc_per_model/len(batches)))
    print("\n")
