...
def extract_details_from_file_name(batches):
    ...
    Example :input [ [1_0_4_filename.jpg,12_0_4_filename.jpg,13_0_2_filename.jpg],[ [R1,G1,B1],[R2,G2,B2],[R3,G3,B3] ] ]
    changes to    [ [[1 0 4],[12 0 4],[13 0 2]],[ [R1,G1,B1],[R2,G2,B2],[R3,G3,B3] ] ]
    :Args
    ...
    for batch in batches:
        batch_details = []
        for name in batch[0]: # get the [1_0_4_filename.jpg,12_0_4_filename.jpg,13_0_2_filename.jpg] part
            age_gender_color_by_regex = re.search('([0-9]{1,3})_([0-9])_([0-9])', name)
            details = [age_gender_color_by_regex.group(i).replace("_", "") for i in range(1, 4)]  # Before the gender and the color there is '_' then we remove it
...
if __name__ == '__main__':
    ...
    X_detail = tf.placeholder(dtype=tf.float32, shape=[None, 3], name='Details')
...
with tf.Session() as sess:
    ...
    for i in range(len(batches)):
        ...
        details = details.reshape([-1, 3])

        batch_img = [batch[0] for _ in range(1, 101, int(years_between_images))]
        details_img = [[year, gender, body_color] for year in range(1, 101, int(years_between_images))]

        # Restore the saved vairable
        saver.restore(sess, './model_92.250046/saved_variable')
