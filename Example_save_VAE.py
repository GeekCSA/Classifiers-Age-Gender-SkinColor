...
if __name__ == '__main__':
    ...
    optimizer = tf.train.AdamOptimizer(0.0005,name="Adam").minimize(loss)
    ...
    with tf.Session() as sess:
        ...
        for i in range(30001):
            ...
            batch = batches[k][1]  # Get pixels matrix of current batch from batches list
            details = np.array(batches[k][0])
            ...
            if i % 400 == 0:
                ...
                if min_loss > local_loss * 1.1 and local_loss < 150:
                    min_loss = local_loss
                    os.makedirs("model_"+str(min_loss))
                    saved_path = saver.save(sess, "model_"+str(min_loss)+'/saved_variable_VAE_Faces_56')
